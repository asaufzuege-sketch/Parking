...
class ParkingViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun launchProvider(session: ParkingSession) {
        val context = getApplication<Application>()
        val adapter = ProviderRegistry.adapterFor(session.provider)
        val result = adapter.buildStartIntent(context, session)
        when (result) {
            is LaunchResult.Success -> {
                _uiState.update { it.copy(pendingLaunchIntent = result.intent) }
                val providerState = ParkingStateMachine.transition(
                    _uiState.value.parkingState, ParkingEvent.ProviderLaunched
                )
                applyStateChange(providerState)
                log(session.id, "ProviderLaunched", session.provider.name)
            }
